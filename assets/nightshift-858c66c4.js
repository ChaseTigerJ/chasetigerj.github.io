...
/* tigOS arcade, Nightshift part 01: the line. Four stations on a loop (Tiger Ave, Canal St, Foundry Yard, Harbor Pl) joined by long tunnels,
  ...
  var TUNNEL = { id:'tunnel', name:'Tunnel', sub:'', band:'#222', accent:'#ff3b3b', wall:'brick', tunnel:true, seg:0, layout:function(F){
      var alc = TUNNEL.seg % 2 ? [[10, 14], [30, 34], [48, 52]] : [[6, 10], [24, 28], [42, 46]];   /* the two stretches place their alcoves differently, so the walk does not repeat */
      alc.forEach(function(r){ F.fill(r[0], r[1], 4, 4, 'W'); F.fill(r[0], r[1], 9, 9, 'W'); }); if(TUNNEL.seg % 2){ F.fill(18, 20, 9, 9, 'W'); F.fill(38, 40, 4, 4, 'W'); }
...
/* tigOS arcade, Nightshift part 02: the world builder. Turns the grid into merged meshes (one per material) plus props, lights and the
  ...
  function buildWorld(scene, opts){
    ...
    if(tunnel){   /* cable runs, signal lights and a soaked floor: the tunnel is its own place */
      ...
      var alc2 = def.seg % 2 ? [[10, 4], [30, 4], [48, 4], [10, 9], [30, 9], [48, 9]] : [[6, 4], [24, 4], [42, 4], [6, 9], [24, 9], [42, 9]];
      alc2.forEach(function(w2, bi){ var bm2 = M.bulb.clone(); world.dispose.push(bm2); var lamp2 = new THREE.Mesh(new THREE.SphereGeometry(.05, 8, 6), bm2); lamp2.position.set(w2[0] + 2.5, CEIL_LO - .3, w2[1] + .5); root.add(lamp2); world.dispose.push(lamp2.geometry); var L2 = new THREE.PointLight(0xffd6a0, .8, 6, 1); L2.position.set(w2[0] + 2.5, CEIL_LO - .5, w2[1] + .5); root.add(L2); world.lights.push({ l:L2, pri:2 }); var cage = box(.6, .2, .3, M.dark, w2[0] + 2.5, CEIL_LO - .12, w2[1] + .5); root.add(cage); world.flick.push({ l:L2, m:bm2, base:.8, on:bi % 2 === 0, t:1 + bi*.7 }); }); }
    ...
    world.props.forEach(function(p){
      ...
      if(k === 'C'){ var cx = p.x + .5, cz = p.z + .5, hh = p.cy - p.fy; var col = new THREE.Group(); col.add(box(.34, hh, .34, M.steel, 0, p.fy + hh/2, 0)); col.add(box(.5, .08, .5, M.steel, 0, p.fy + .04, 0)); col.add(box(.5, .12, .5, M.steel, 0, p.cy - .06, 0)); col.add(box(.06, hh - .3, .42, M.dark, 0, p.fy + hh/2, 0)); col.add(box(.42, hh - .3, .06, M.dark, 0, p.fy + hh/2, 0)); col.position.set(cx, 0, cz); root.add(col); }
        ...
        var wk = def.wall === 'brick' ? 'brick' : 'tile', bx0 = fc.x, bz0 = fc.z, us = wk === 'brick' ? .5 : 2;   /* the shell goes into the late batch: one merged mesh per material for all the stairwells of a station instead of eleven boxes each */
        lbox(wk, bx0 + dx*.65 + dz*.5, p.fy + hh2/2, bz0 + dz*.65 + dx*.5, dx ? 1.3 : .06, hh2, dx ? .06 : 1.3, us); lbox(wk, bx0 + dx*.65 - dz*.5, p.fy + hh2/2, bz0 + dz*.65 - dx*.5, dx ? 1.3 : .06, hh2, dx ? .06 : 1.3, us); lbox(wk, bx0 + dx*1.3, p.fy + hh2/2, bz0 + dz*1.3, dx ? .06 : 1, hh2, dx ? 1 : .06, us);
        lbox('ceiling', bx0 + dx*.65, p.fy + hh2 - .03, bz0 + dz*.65, dx ? 1.3 : 1, .06, dx ? 1 : 1.3, .5); lbox('floor', bx0 + dx*.65, p.fy + .02, bz0 + dz*.65, dx ? 1.3 : 1, .04, dx ? 1 : 1.3, 1);
        for(var st = 0; st < 5; st++) lbox('floor', bx0 + dx*(.2 + st*.22), p.fy + .11 + st*.22, bz0 + dz*(.2 + st*.22), dx ? .28 : .96, .22, dx ? .96 : .28, 1);   /* steps rising away into the dark */
   ...
   the DOM HUD over the canvas, the title card with its settings, input, and the peek/dbg hooks the test suite drives. Closes nightshift(). */
    ...
    (function build(){
      ...
      function applyGfx(){ var tier = soft ? 'low' : SET.gfx, dpr = window.devicePixelRatio || 1, pr = soft ? .3 : { low:.55, medium:.8, high:Math.min(1.5, dpr), ultra:Math.min(2, dpr) }[tier]; renderer.setPixelRatio(pr); composer.setPixelRatio(pr);
        bloom.enabled = tier !== 'low' || /[?&]gl=1/.test(location.search); bloom.strength = tier === 'ultra' ? .28 : .22; applyLights(); if(vw > 2) R.resize(vw, vh); }
      ...
      R = { scene:scene, camera:camera, renderer:renderer, world:world, train:TR, fresh:true, soft:function(){ return soft; }, bloom:bloom, cache:cache,
        ...
        warmActors:function(){ var tmp = new THREE.Group(), bodies = []; ['walker', 'runner', 'brute', 'mta', 'nypd', 'conductor', 'wraith'].forEach(function(k){ try { var m = makeZombie(0, false, k); m.grp.traverse(function(o){ o.visible = true; }); bodies.push(m); tmp.add(m.grp); } catch(e){} });   /* every optional part shown: compile only visits visible meshes, and a torn sleeve or ribs that happened to be hidden here would otherwise compile on its first spawn, possibly mid-seam */
          ...
          if(R.warm){ scene.remove(R.warm); R.warm.traverse(function(o){ if(o.isMesh || o.isSprite){ [].concat(o.material).forEach(function(mt){ mt.dispose(); }); } }); } tmp.position.set(MW/2, -50, MH/2); scene.add(tmp); R.warm = tmp; compileScene();
        ...
        menu:function(on){ api.canvas.classList.toggle('cctv', on); if(hudEl) hudEl.hidden = true; if(menuEl){ menuEl.hidden = !on; if(on) showPanel('main'); } },
    ...
    var onModel = function(k, sc){ if(!R) return; if(k === 'train') R.train.setModel(sc); if(k === 'zombie' || k === 'wraith') R.warmActors(); if(k === 'props') R.invalidate(); }; MODELS.listeners.push(onModel);
