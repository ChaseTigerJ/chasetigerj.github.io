...
(function(){
  ...
  function assets(){
    ...
    M.void = new THREE.MeshBasicMaterial({ color:0x04040a }); M.step = new THREE.MeshBasicMaterial({ color:0x0d0e14 });   /* the stairwell shell: unlit, near black, so the opening reads as a void going up into the dark */
...
/* tigOS arcade, Nightshift part 01: the line. Four stations on a loop (Tiger Ave, Canal St, Foundry Yard, Harbor Pl) joined by long tunnels,
  ...
  var STATION = { x0:9, x1:50, z0:1, z1:15 }, PIT = { z0:5, z1:8 }, TRACK_Z = 7, PIT_Y = -1.05, CEIL_HI = 3.4, CEIL_LO = 2.6, CEIL_TRK = 3.15, TRAIN_FLOOR = PIT_Y + .95;   /* CEIL_TRK: the bore over the track bed outside the hall, tall enough for the car roof and its AC humps (2.86 m) */   /* the car floor: a short step down from the platform, 2 m of headroom under the roof */
  ...
  var TUNNEL = { id:'tunnel', name:'Tunnel', sub:'', band:'#222', accent:'#ff3b3b', wall:'brick', tunnel:true, seg:0, layout:function(F){
      F.fill(0, MW - 1, 4, 4, 'W'); F.fill(0, MW - 1, 9, 9, 'W');   /* a bench walkway a step up along both walls, the whole way: straight walls, nowhere for the train to hit you if you are up on it (the old recessed alcoves left wall stubs standing in the tunnel like pillars) */
      F.machines([]); F.start(2.5, 8.5, 0); } };
  var TUNNEL_SEGS = 2;   /* tunnel chunks between two stations: ~120 m on foot, walked as one place */
  function genMap(def, seg){
    CUR = def; def.seg = seg || 0; MAP = []; for(var z = 0; z < MH; z++){ var row = []; for(var x = 0; x < MW; x++) row.push('#'); MAP.push(row); }
    ...
    if(def.tunnel){ fill(0, MW - 1, PIT.z0, PIT.z1, '_'); def.layout(F); }
      ...
      def.layout(F);
      for(var zz = PIT.z0; zz <= PIT.z1; zz++){ put(0, zz, zz === PIT.z0 ? '#' : 'F'); put(MW - 1, zz, zz === PIT.z0 ? '#' : 'F'); }   /* the fence closes the north walkway only; the rails and the south side run open into the tunnel (a wall across the track bed had the train driving through brick) */
    ...
    for(var sz = 0; sz < MH; sz++) for(var sx = 0; sx < MW; sx++){ var c0 = MAP[sz][sx]; if(c0 !== 'S' && c0 !== 'F') continue;
      [[0, 1], [0, -1], [1, 0], [-1, 0]].forEach(function(o){ var nx = sx + o[0], nz = sz + o[1]; if(MAP[nz] && isFloor(MAP[nz][nx]) && MAP[nz][nx] !== 'F' && !MTILE[nx+','+nz]) SPAWNS.push({ x:nx + .5, y:nz + .5, fx:c0 === 'S' ? sx + .5 - o[0]*.6 : sx + .5 + (sx === 0 ? -2.5 : 2.5), fy:c0 === 'S' ? sz + .5 - o[1]*.6 : sz + .5, stairs:c0 === 'S' }); }); }   /* a stair spawn starts at the top step, 1.1 m into the alcove, and comes down over the sandbags */
    ...
    CEILH = []; FLOORY = [];
    for(var z3 = 0; z3 < MH; z3++){ CEILH.push([]); FLOORY.push([]); for(var x3 = 0; x3 < MW; x3++){ var inHall = !def.tunnel && x3 >= STATION.x0 && x3 <= STATION.x1 && z3 >= STATION.z0 && z3 <= STATION.z1; CEILH[z3].push(inHall ? CEIL_HI : z3 >= PIT.z0 && z3 <= PIT.z1 ? CEIL_TRK : CEIL_LO); FLOORY[z3].push(MAP[z3][x3] === '_' || MAP[z3][x3] === 'F' ? PIT_Y : 0); } }
...
/* tigOS arcade, Nightshift part 02: the world builder. Turns the grid into merged meshes (one per material) plus props, lights and the
   ...
   and returns handles the game animates. Built worlds are kept: moving down the line detaches the old root from the scene (world.detach) and
   re-attaches a cached one when you come back (world.attach), so a chunk swap costs no geometry, no textures and no shader compiles. The train
   is built once at scene level (buildTrain) and shared by every chunk. */
  ...
  function buildWorld(scene, opts){
    ...
    var world = { root:root, def:def, lights:[], tubes:[], props:[], doors:{}, buys:{}, machines:{}, power:null, sandbags:[], dispose:[], signals:[], flick:[], textures:[SM.band.map, SM.sign.map], add:function(o){ root.add(o); return o; } };   /* textures: what the renderer uploads ahead of the first visit */
    ...
    if(!tunnel){   /* chain-link fences close the tunnels at the map's edge; somebody forced the last panel: that is where they come in, and where you go out */
      [0.5, MW - .5].forEach(function(fx){ var fg = new THREE.PlaneGeometry(1.0, CEIL_LO - PIT_Y, 4, 8), fm = new THREE.Mesh(fg, M.fence); fm.position.set(fx, (CEIL_LO + PIT_Y)/2, PIT.z0 + .5); fm.rotation.y = Math.PI/2; root.add(fm); world.dispose.push(fg); var bent = new THREE.Mesh(new THREE.PlaneGeometry(1.3, 1.6, 4, 4), M.fence); bent.position.set(fx + (fx < 2 ? .5 : -.5), PIT_Y + .8, PIT.z0 + 3.3); bent.rotation.y = Math.PI/2 + (fx < 2 ? .9 : -.9); bent.rotation.z = .3; root.add(bent); world.dispose.push(bent.geometry);   /* one panel still up over the walkway, one torn down and lying by the rails; the rails themselves run clear */
        for(var pz2 = PIT.z0; pz2 <= PIT.z0 + 1; pz2++){ var pg = new THREE.CylinderGeometry(.03, .03, CEIL_LO - PIT_Y, 6), pm = new THREE.Mesh(pg, M.rail); pm.position.set(fx, (CEIL_LO + PIT_Y)/2, pz2); root.add(pm); world.dispose.push(pg); } }); }
    var matFor = { tile:M.tile, brick:M.brick, floor:M.floor, walk:M.metal, ceiling:M.ceiling, ballast:M.ballast, edge:M.floor, dark:M.dark, void:M.void, step:M.step };
    ...
    var textTex = function(w, h, fn){ var c = mk(w, h); fn(c.getContext('2d'), w, h); var t = new THREE.CanvasTexture(c); t.colorSpace = THREE.SRGBColorSpace; world.dispose.push(t); world.textures.push(t); return t; };
    /* chalk: a stroke that wobbles like a hand did it, drawn twice at low alpha so it dusts */
    var chalkPath = function(x, pts, close){ for(var pass = 0; pass < 2; pass++){ x.beginPath(); pts.forEach(function(q, i){ var jx = q[0] + (srand() - .5)*2.2, jy = q[1] + (srand() - .5)*2.2; if(i) x.lineTo(jx, jy); else x.moveTo(jx, jy); }); if(close) x.closePath(); x.strokeStyle = pass ? 'rgba(244,241,234,.55)' : 'rgba(244,241,234,.95)'; x.lineWidth = pass ? 4.5 : 2.6; x.lineJoin = 'round'; x.lineCap = 'round'; x.stroke(); } };
    var chalkBox = function(x, x0, y0, w, h){ x.setLineDash([9, 7]); chalkPath(x, [[x0, y0], [x0 + w, y0], [x0 + w, y0 + h], [x0, y0 + h]], true); x.setLineDash([]); };
    /* the gun silhouettes, in a 100 x 40 box: receiver, barrel, grip, magazine, stock and the odd sight, per class of weapon */
    var CHALK = { smg:[[[18,14],[62,14],[62,25],[18,25]], [[62,17],[90,17],[90,21],[62,21]], [[28,25],[37,25],[39,38],[30,38]], [[46,25],[54,25],[52,40],[44,40]], [[18,16],[5,17],[5,22],[18,23]], [[40,10],[44,10],[44,14],[40,14]]],
      rifle:[[[22,14],[64,14],[64,24],[22,24]], [[64,16],[98,16],[98,20],[64,20]], [[30,24],[38,24],[40,37],[32,37]], [[46,24],[55,24],[58,38],[49,38]], [[22,15],[2,18],[2,26],[22,24]], [[38,9],[58,9],[58,14],[38,14]], [[66,14],[68,10],[70,14]]],
      shotgun:[[[26,15],[60,15],[60,25],[26,25]], [[60,16],[98,16],[98,20],[60,20]], [[62,20],[88,20],[88,24],[62,24]], [[26,15],[3,19],[3,28],[26,25]], [[32,25],[40,25],[42,36],[34,36]], [[66,11],[70,11],[70,16],[66,16]]],
      sniper:[[[24,15],[58,15],[58,24],[24,24]], [[58,17],[99,17],[99,20],[58,20]], [[30,24],[38,24],[40,36],[32,36]], [[44,24],[50,24],[50,33],[44,33]], [[24,15],[4,17],[4,27],[24,24]], [[34,8],[56,8],[56,13],[34,13]], [[38,13],[40,15]], [[52,13],[54,15]], [[82,20],[80,30]], [[90,20],[92,30]]],
      lmg:[[[18,13],[64,13],[64,26],[18,26]], [[64,16],[96,16],[96,21],[64,21]], [[28,26],[37,26],[39,38],[30,38]], [[40,26],[58,26],[58,38],[40,38]], [[18,15],[4,17],[4,25],[18,25]], [[40,8],[52,8],[52,13],[40,13]], [[74,21],[70,34]], [[80,21],[84,34]]] };
    var CHALK_OF = { wasp:'smg', stitcher:'smg', reaper:'rifle', doorman:'shotgun', longbow:'sniper', anvil:'lmg' };
    var chalkGun = function(x, id, x0, y0, w, h){ var parts = CHALK[CHALK_OF[id] || 'smg']; parts.forEach(function(poly){ chalkPath(x, poly.map(function(q){ return [x0 + q[0]/100*w, y0 + q[1]/40*h]; }), poly.length > 2); }); };
    ...
    if(tunnel){   /* cable runs, signal lights and a soaked floor: the tunnel is its own place */
      ...
      /* the only light down here: caged bulbs on a bad circuit, each on its own stutter (world.flick, driven in part 06), so the walk is mostly dark with a lamp coming and going */
      var alc2 = def.seg % 2 ? [[10, 4], [30, 4], [48, 4], [20, 9], [40, 9], [56, 9]] : [[6, 4], [26, 4], [46, 4], [16, 9], [36, 9], [52, 9]];   /* bulb positions along the two walkways, staggered so no two face each other */
      alc2.forEach(function(w2, bi){ var bm2 = M.bulb.clone(); world.dispose.push(bm2); var lamp2 = new THREE.Mesh(new THREE.SphereGeometry(.05, 8, 6), bm2); lamp2.position.set(w2[0] + .5, CEIL_LO - .3, w2[1] + .5); root.add(lamp2); world.dispose.push(lamp2.geometry); var L2 = new THREE.PointLight(0xffd6a0, .8, 6, 1); L2.position.set(w2[0] + .5, CEIL_LO - .5, w2[1] + .5); root.add(L2); world.lights.push({ l:L2, pri:2 }); var cage = box(.6, .2, .3, M.dark, w2[0] + .5, CEIL_LO - .12, w2[1] + .5); root.add(cage); world.flick.push({ l:L2, m:bm2, base:.8, on:bi % 2 === 0, t:1 + bi*.7 }); }); }
    /* a late batch for prop shells built after the main flush (stairwells): axis-aligned boxes as six quads into one Geo per material */
    var late = {}, LG = function(k){ return late[k] || (late[k] = new Geo()); };
    var lbox = function(k, cx, cy, cz, w, h, d, us){ var x0 = cx - w/2, x1 = cx + w/2, y0 = cy - h/2, y1 = cy + h/2, z0 = cz - d/2, z1 = cz + d/2, g = LG(k);
      g.quad([x0, y0, z0], [x1, y0, z0], [x1, y1, z0], [x0, y1, z0], [[x0*us, y0*us], [x1*us, y0*us], [x1*us, y1*us], [x0*us, y1*us]], [0, 0, -1]); g.quad([x0, y0, z1], [x1, y0, z1], [x1, y1, z1], [x0, y1, z1], [[x0*us, y0*us], [x1*us, y0*us], [x1*us, y1*us], [x0*us, y1*us]], [0, 0, 1]);
      g.quad([x0, y0, z0], [x0, y0, z1], [x0, y1, z1], [x0, y1, z0], [[z0*us, y0*us], [z1*us, y0*us], [z1*us, y1*us], [z0*us, y1*us]], [-1, 0, 0]); g.quad([x1, y0, z0], [x1, y0, z1], [x1, y1, z1], [x1, y1, z0], [[z0*us, y0*us], [z1*us, y0*us], [z1*us, y1*us], [z0*us, y1*us]], [1, 0, 0]);
      g.quad([x0, y1, z0], [x1, y1, z0], [x1, y1, z1], [x0, y1, z1], [[x0*us, z0*us], [x1*us, z0*us], [x1*us, z1*us], [x0*us, z1*us]], [0, 1, 0]); g.quad([x0, y0, z0], [x1, y0, z0], [x1, y0, z1], [x0, y0, z1], [[x0*us, z0*us], [x1*us, z0*us], [x1*us, z1*us], [x0*us, z1*us]], [0, -1, 0]); };
    world.props.forEach(function(p){
      ...
      if(k === 'C'){ var cx = p.x + .5, cz = p.z + .5, hh = p.cy - p.fy; var col = new THREE.Group(); col.add(box(.34, hh, .34, M.steel, 0, p.fy + hh/2, 0)); col.add(box(.5, .08, .5, M.steel, 0, p.fy + .04, 0)); col.add(box(.5, .12, .5, M.steel, 0, p.cy - .06, 0)); col.add(box(.06, hh - .3, .42, M.dark, 0, p.fy + hh/2, 0)); col.add(box(.42, hh - .3, .06, M.dark, 0, p.fy + hh/2, 0)); col.position.set(cx, 0, cz); root.add(col); }
        ...
        /* the painted sign: spray-stencilled chevrons and THIS WAY on the slats, the way a Zombies map marks a door you can buy through. Both faces. */
        var pt = textTex(256, 384, function(x, w, hh4){ x.clearRect(0, 0, w, hh4); x.fillStyle = '#ffd166'; for(var cv = 0; cv < 3; cv++){ var cy = 62 + cv*56; x.beginPath(); x.moveTo(52, cy + 44); x.lineTo(128, cy); x.lineTo(204, cy + 44); x.lineTo(204, cy + 70); x.lineTo(128, cy + 26); x.lineTo(52, cy + 70); x.closePath(); x.fill(); }
          x.font = '900 46px '+FONT; x.textAlign = 'center'; x.textBaseline = 'middle'; x.fillText('THIS', 128, 266); x.fillText('WAY', 128, 314); x.font = '800 24px '+FONT; x.fillText(DOOR_COST[k]+' PTS', 128, 354);
          for(var sp = 0; sp < 900; sp++){ x.fillStyle = 'rgba(255,209,102,'+(srand()*.35)+')'; x.fillRect(srand()*w, srand()*hh4, 1 + srand()*2, 1 + srand()*2); } for(var dr = 0; dr < 7; dr++){ var dx0 = 40 + srand()*176, dy0 = 90 + srand()*260; x.fillStyle = 'rgba(255,209,102,.8)'; x.fillRect(dx0, dy0, 3, 12 + srand()*40); }
          x.globalCompositeOperation = 'destination-out'; for(var sc2 = 0; sc2 < 260; sc2++){ x.fillStyle = 'rgba(0,0,0,'+(.3 + srand()*.7)+')'; x.fillRect(srand()*w, srand()*hh4, 2 + srand()*5, 1 + srand()*2); } x.globalCompositeOperation = 'source-over'; }), pmat = new THREE.MeshBasicMaterial({ map:pt, transparent:true, polygonOffset:true, polygonOffsetFactor:-1 });
        if(gate){ gate.scale.set(1, h/2.5, 1); gate.rotation.y = vert ? 0 : Math.PI/2; d.add(gate);   /* the model spans X and is thin in Z: it stands across a north-south passage unturned, and turns a quarter for an east-west one */
          var slats = gate.getObjectByName('Slats'), lock = gate.getObjectByName('Lock'), plaque = gate.getObjectByName('Plaque'); if(plaque){ plaque.traverse(function(o){ if(o.isMesh) o.material = lm; }); var p2 = plaque.clone(); p2.rotation.y = Math.PI; p2.position.z = -plaque.position.z; gate.add(p2); entry.signs = [plaque, p2]; }
        ...
        var pa = plane(.9, 1.35, pmat, vert ? 0 : .085, .95, vert ? .085 : 0, vert ? [0, 1] : [1, 0]), pb = plane(.9, 1.35, pmat, vert ? 0 : -.085, .95, vert ? -.085 : 0, vert ? [0, -1] : [-1, 0]); d.add(pa); d.add(pb); entry.signs = entry.signs.concat([pa, pb]); entry.paint = [pa, pb];
        ...
        /* the stairwell: tiled sides and back, a concrete floor and ceiling, steps rising away, an exit lamp over the sign. It used to be a black box, which read from the
           menu camera as the wall stopping dead beside two floating EXIT signs */
        var bx0 = fc.x, bz0 = fc.z, px = dz ? 1 : 0, pz = dx ? 1 : 0, nbA = ch(p.wx + px, p.wz + pz) === 'S', nbB = ch(p.wx - px, p.wz - pz) === 'S';   /* neighbouring stair tiles share one opening: no divider wall between them (two coplanar dividers used to z-fight, the shimmer on the menu camera) */
        /* the shell is a void: unlit near-black sides, back and ceiling, so the opening reads as a stairwell going up into the dark under a lit EXIT sign, not a lit tiled room */
        if(!nbA) lbox('void', bx0 + dx*.65 + px*.5, p.fy + hh2/2, bz0 + dz*.65 + pz*.5, dx ? 1.3 : .06, hh2, dx ? .06 : 1.3, 1); if(!nbB) lbox('void', bx0 + dx*.65 - px*.5, p.fy + hh2/2, bz0 + dz*.65 - pz*.5, dx ? 1.3 : .06, hh2, dx ? .06 : 1.3, 1); lbox('void', bx0 + dx*1.3, p.fy + hh2/2, bz0 + dz*1.3, dx ? .06 : 1, hh2, dx ? 1 : .06, 1);
        lbox('void', bx0 + dx*.65, p.fy + hh2 - .03, bz0 + dz*.65, dx ? 1.3 : 1, .06, dx ? 1 : 1.3, 1); lbox('void', bx0 + dx*.65, p.fy + .02, bz0 + dz*.65, dx ? 1.3 : 1, .04, dx ? 1 : 1.3, 1);
        for(var st = 0; st < 5; st++) lbox('step', bx0 + dx*(.2 + st*.22), p.fy + .11 + st*.22, bz0 + dz*(.2 + st*.22), dx ? .28 : .96, .22, dx ? .96 : .28, 1);   /* steps rising away into the dark, barely there */
        var xl = new THREE.Mesh(new THREE.SphereGeometry(.045, 8, 6), M.bulb); xl.position.set(dx*.35, p.cy - p.fy - .12, dz*.35); sg.add(xl); world.dispose.push(xl.geometry);   /* the lamp is emissive only: eight more point lights a station is a real cost on every fragment, and the hall lights reach in far enough */
        ...
        /* the wall buy is a chalk drawing of the gun, its name and its price, the way the original Zombies maps mark them: no model hangs on the wall */
        var bt = textTex(256, 160, function(x, w, h4){ x.clearRect(0, 0, w, h4); chalkBox(x, 8, 8, w - 16, h4 - 16); text(x, wp.name.toUpperCase(), 128, 34, 28, '#f4f1ea', 'center', 900); chalkGun(x, wid, 40, 54, 176, 56); text(x, wp.cost+' PTS', 128, 136, 26, '#ffd166', 'center', 900); for(var cd = 0; cd < 500; cd++){ x.fillStyle = 'rgba(244,241,234,'+(srand()*.12)+')'; x.fillRect(srand()*w, srand()*h4, 1, 1); } }), bm = new THREE.MeshBasicMaterial({ map:bt, transparent:true }), bp = plane(1.5, .95, bm, fc.x, 1.65, fc.z, fc.dir); root.add(bp); world.buys[p.wx+','+p.wz] = bp; }
    ...
    Object.keys(late).forEach(function(k){ var g = late[k].build(), m = new THREE.Mesh(g, matFor[k]); m.matrixAutoUpdate = false; root.add(m); world.dispose.push(g); });   /* the stairwell shells, one mesh per material */
    /* bins for life */
    ...
    root.add(new THREE.HemisphereLight(0x303848, 0x0c0a08, tunnel ? .08 : .45));
    root.add(new THREE.AmbientLight(0x1a1c24, tunnel ? .12 : .6));   /* the tunnel gets almost no fill: what you see is what a bulb happens to light */

    /* ---------- the chunk's life: attach on arrival (or return), detach on leaving, destroy only when the game closes ----------
       The map globals (MACHINES, BOXM) are remade by genMap every time a chunk loads, so on attach the world's render handles are rebound to
       the current machine objects and the box is shown or not as the run has it. */
    world.attach = function(machines, o){ if(root.parent !== scene) scene.add(root); o = o || {};
      (machines || []).forEach(function(m){ var wm = world.machines[m.kind]; if(!wm || wm === m) return; Object.keys(wm).forEach(function(k){ if(!(k in m)) m[k] = wm[k]; }); world.machines[m.kind] = m; });
      var bx = world.machines.box; if(bx){ bx.here = !!o.boxHere; if(bx.model){ bx.chest.children.forEach(function(c2){ if(c2 !== bx.pedestal && c2 !== bx.beam && c2 !== bx.bear) c2.visible = bx.here; }); bx.pedestal.visible = !bx.here; bx.beam.visible = false; bx.bear.visible = false; } else { bx.chest.visible = bx.here; bx.pedestal.visible = !bx.here; } } };
    world.detach = function(){ scene.remove(root); };
  ...
  /* ---------- the train: four cars, lit windows, headlights. Built once at scene level and shared by every chunk; it lives in the tunnel until the
     game calls it. The Blender consist (blender/train.py, an R160-style NYC set) replaces the box stand-in the moment its .glb arrives; the group,
     nose, tail and lights stay. Its lights sit in their own always-visible group that follows the train, so the scene's light count never changes
     when the train appears: a changed light count makes Three.js recompile every shader, and that was the hitch when the train rolled in. ---------- */
  function buildTrain(scene){
    var THREE = window.THREE, A = assets(), M = A.mat, T = A.tex, dispose = [];
    var box = function(w, h, d, mat, x, y, z){ var g = new THREE.BoxGeometry(w, h, d), m = new THREE.Mesh(g, mat); m.position.set(x, y, z); dispose.push(g); return m; };
    var cyl = function(r0, r1, h, mat, x, y, z, seg){ var g = new THREE.CylinderGeometry(r0, r1, h, seg || 12), m = new THREE.Mesh(g, mat); m.position.set(x, y, z); dispose.push(g); return m; };
    var plane = function(w, h, mat, x, y, z, dir){ var g = new THREE.PlaneGeometry(w, h), m = new THREE.Mesh(g, mat); m.position.set(x, y, z); m.rotation.y = Math.atan2(dir[0], dir[1]); dispose.push(g); return m; };
    var tr = new THREE.Group(); var carL = 9.2, cars = 4, nose = (cars - 1)*(carL + .3) + carL/2, stand = new THREE.Group(); tr.add(stand);
    for(var ci = 0; ci < cars; ci++){ var cx0 = ci*(carL + .3); var body = box(carL, 2.75, 2.7, M.train, cx0, PIT_Y + 1.78, 0); stand.add(body); stand.add(box(carL, .3, 2.72, M.trainStripe, cx0, PIT_Y + 1.1, 0)); stand.add(box(carL - .6, .5, 2.5, M.dark, cx0, PIT_Y + 3.3, 0)); stand.add(box(carL, .5, 2.2, M.dark, cx0, PIT_Y + .3, 0));
      for(var wi = 0; wi < 5; wi++){ var wx = cx0 - carL/2 + .9 + wi*1.85; stand.add(box(.95, .62, 2.74, M.window, wx, PIT_Y + 2.3, 0)); } for(var wh = 0; wh < 2; wh++){ stand.add(cyl(.42, .42, .3, M.dark, cx0 - carL/2 + 1.5 + wh*(carL - 3), PIT_Y + .42, .8, 10)); stand.add(cyl(.42, .42, .3, M.dark, cx0 - carL/2 + 1.5 + wh*(carL - 3), PIT_Y + .42, -.8, 10)); } }
    ...
    /* the lights ride in their own group (see above): the headlight, the warm cabin light that follows you down the consist, one strip per car */
    var lights = new THREE.Group(), hlight = new THREE.PointLight(0xfff4e0, 0, 30, 1); hlight.position.set(nose + 1.5, PIT_Y + 1.4, 0); lights.add(hlight);
    var cabin = new THREE.PointLight(0xfff0dc, 0, 12, 1); cabin.position.set(nose/2, PIT_Y + 2.2, 0); lights.add(cabin); var carLights = [];
    ...
    for(var ci2 = 0; ci2 < cars; ci2++){ var cx2 = ci2*(carL + .3), fy0 = TRAIN_FLOOR, cy0 = PIT_Y + 3.4;   /* 2.45 m of headroom over the car floor (the shell is 3.5 m above the rail): a 1.6 m eye plus bob clears the door headers at 2.95 */
      ...
      var carL2 = new THREE.PointLight(0xe8ecff, 0, 8, 1); carL2.position.set(cx2, cy0 - .25, 0); lights.add(carL2); carLights.push(carL2); }
    tr.add(inner);
    tr.position.set(-200, 0, TRACK_Z); tr.visible = false; scene.add(tr); lights.position.set(-200, 0, TRACK_Z); scene.add(lights);
    var train = { grp:tr, lights:lights, stand:stand, tail:-carL/2, nose:nose, hlight:hlight, cabin:cabin, inner:inner, carLights:carLights, model:false, doors:[], carL:carL, cars:cars, floorY:TRAIN_FLOOR };
    train.setModel = function(sc){ if(train.model) return; stand.visible = false; sc.position.set(0, PIT_Y, 0); tr.add(sc); train.model = true; train.doors = []; sc.traverse(function(o){ if(/^Door[LR]/.test(o.name) && !o.isMesh){   /* the leaf empties only: their meshes carry the same name prefix and must ride along, not slide twice */ if(o.userData.x0 === undefined) o.userData.x0 = o.position.x; o.userData.side = o.name.charAt(4) === 'L' ? -1 : 1; train.doors.push(o); } }); };   /* the glb is modelled with car 0 at x=0 and rails at y=0, along +X, so it drops straight onto the stand-in's frame */
    if(MODELS.train) train.setModel(MODELS.train);
    train.dispose = function(){ if(MODELS.train && MODELS.train.parent === tr) tr.remove(MODELS.train); scene.remove(tr); scene.remove(lights); dispose.forEach(function(g){ g.dispose(); }); };
    return train;
  }
/* tigOS arcade, Nightshift part 03: actors. The dead (the Blender humanoid in blender/humanoid.py, dressed per kind, with the jointed capsule
  ...
  function makeHuman(v, kind){
    ...
    show(['HardHat', 'Cap', 'Vest', 'Badge', 'Belt', 'Coat', 'Lantern', 'Hair', 'Ribs', 'Tatters', 'WoundHead', 'WoundBelly', 'SleeveTornR', 'ArmSkinR', 'ArmGoreR', 'ShinTornL', 'ShinSkinL', 'ShinGoreL'], false);
    var uniform = kind === 'mta' || kind === 'nypd';
    ...
    /* the damage: every body torn a little differently. The crowd is the worst of it; the transit dead kept most of the uniform; the Conductor kept his coat. */
    var dmgK = cond ? .15 : uniform ? .45 : 1;
    if(Math.random() < .75*dmgK && !uniform && !cond) show(['Tatters'], true); if(Math.random() < .55*dmgK && !uniform && !cond) show(['Ribs'], true);
    if(Math.random() < .45*dmgK) show(['WoundHead'], true); if(Math.random() < .5*dmgK && !cond) show(['WoundBelly'], true);
    if(Math.random() < .55*dmgK && !cond){ show(['UpperArmR'], false); show(['SleeveTornR', 'ArmSkinR', 'ArmGoreR'], true); }
    if(Math.random() < .45*dmgK && !cond){ show(['ShinL'], false); show(['ShinTornL', 'ShinSkinL', 'ShinGoreL'], true); }
    var mats = g.userData.mats; mats.forEach(function(m){ var n = m.name;
      ...
      else if(n === 'Coat'){ m.color.set(0x1c1f2e); } else if(n === 'Cap'){ m.color.set(cond ? 0x1c1f2e : 0x161c33); } else if(n === 'Eyes'){ m.emissive.setHex(cond ? 0xffb050 : brute ? 0xff3020 : 0xffae1a); m.emissiveIntensity = cond ? 4 : 3.2; m.color.setHex(cond ? 0xffb050 : brute ? 0xff3020 : 0xffae1a); }   /* the lit eyes of the Zombies maps: yellow on the crowd, red on a brute, the Conductor's lantern amber */
      m.needsUpdate = true; });
    ...
    var lantern = cond && by.Lantern ? by.Lantern : null;   /* the renderer's one lantern light follows this node; a light per body would change the scene's light count */
    ...
    return { grp:body, inner:g, P:P, mats:mats, s:s, r:(brute ? .48 : .34)*s, h:1.9*s, headY:1.5*s, lop:cond ? .05 : Math.random()*.25, tilt:(Math.random() < .5 ? -1 : 1)*(.08 + Math.random()*.14), hunch:cond ? .04 : .12 + Math.random()*.16, glb:true, human:true, lantern:lantern, kind:kind };
  }
  ...
  function makeGun(id, up, opts){
    ...
    var bare = !!(opts && opts.bare), fl = new THREE.Mesh(new THREE.PlaneGeometry(.34, .34), A.mat.flash); fl.position.set(mz.x + .1, mz.y, mz.z); fl.rotation.y = Math.PI/2; fl.visible = false; g.add(fl); geos.push(fl.geometry);
    var fl2 = fl.clone(); fl2.rotation.set(0, 0, Math.PI/4); fl2.position.set(mz.x + .16, mz.y, mz.z); fl2.visible = false; g.add(fl2);
    /* the held gun always carries its two lights (the ember of a forged gun at 0 until it is forged) and a display copy carries none: swapping guns or
       spinning the box must never change the scene's light count, or every shader recompiles */
    var ML = new THREE.PointLight(0xffb060, 0, 6, 1); ML.position.copy(mz); if(!bare) g.add(ML);
    if(!bare){ var L = new THREE.PointLight(0xff8a00, up ? .5 : 0, 1.5, 1); L.position.set(.1, .06, 0); g.add(L); }
...
/* tigOS arcade, Nightshift part 05: the rules. Waves, points, the economy (grates, wall buys, resupply, the box, perks, power, the Forge),
  ...
  function nightshift(api){
    ...
    var special, fogK, P, zombies, pickups, grenades, wave, toSpawn, brutes, spawnT, alive, kills, points, total, hp, maxhp, regenT, doors, power, perks, weapons, slot, fireT, reloadT, reloading, held, mouseDown, trigger, shake, flash, hitM, hitKill, dmgFlash, whiteFlash, banner, bannerT, over, puX2, puInsta, gren, grenT, meleeT, melee, msg, msgT, boxRoll, boxName, boxLast, boxPick, swapT, prompt, t, bob, stepPh, between, recoil, dragX, dragY, flick = 0, flickT = 4, flickTube = null, dripT = 5, train, footY, overMsg, mode = 'menu', booted = false, jumpZ = 0, vz = 0, slowT = 0, attract = null, lastRun = null;
    var stationIdx = 0, rideOff = 0, boxBear = false, boxSpin = null, heads = 0, shots = 0, rode = 0, walked = 0, chunk = null, opened = {}, boxAt = 0, boxUses = {}, bearT = 0, riding = 0, rideT = 0, rideFrom = 0, transit = 0, transitT = 0, conductor = null, condT = 0, passT = 0, prewarmT = 1.2, actorsWarm = false, fade = 0, fadeTo = 0, dying = 0, endT = 0, ended = false, stats = null, visited = {}, doorsOpenK = 0, chime = 0, spinUp = 0;
    var R = null;   /* the renderer bag, built in part 06 */
    function walk(x, z){ var c = ch(x, z); if(c === '.' || c === 'P' || c === 'C' || c === 'W') return !MTILE[x+','+z]; if(c === 'F') return true; if(c === '_'){ if(!trainBlocks(x)) return true; if(!trainOpen()) return false; var dz0 = Math.abs(z + .5 - TRACK_Z); return dz0 < 1.1 || (dz0 < 1.6 && doorAt(x)); } if(isDoor(c)) return !!doors[x+','+z]; return false; }   /* a standing car with its doors open: the two interior rows are its floor, and the gap rows beside it are crossable only at a door opening, so the shell is solid for everyone (the dead used to walk in through the side and stand a metre down inside the floor) */
    ...
    function doorAt(x){ if(!train) return false; var cx = x + .5 - train.x; for(var ci = 0; ci < 4; ci++){ var c0 = ci*9.5; if(Math.abs(cx - (c0 - 2.3)) < 1.05 || Math.abs(cx - (c0 + 2.3)) < 1.05) return true; } return false; }   /* the door openings: two a side per car, 1.5 m wide at +-2.3 from each car's centre, cars 9.5 m apart from the consist origin (see buildTrain) */
    function inCar(x, y){ return !!(train && train.grp.visible && Math.abs(y - TRACK_Z) < 1.35 && x > train.x + train.tail + .2 && x < train.x + train.nose - .2); }
    ...
    function computeFlow(){ var px = P.x|0, py = P.y|0, key = px+','+py+':'+Object.keys(doors).length+':'+(train && train.grp.visible && train.v < 1.5 ? (train.x|0) : 'n'); if(key === flowKey) return; flowKey = key; flow.fill(-1); var q = [px + py*MW]; flow[q[0]] = 0; for(var h = 0; h < q.length; h++){ var i = q[h], x = i % MW, y = (i / MW)|0, d = flow[i] + 1; [[1,0],[-1,0],[0,1],[0,-1]].forEach(function(o){ var nx = x + o[0], ny = y + o[1], j = nx + ny*MW; if(nx < 0 || ny < 0 || nx >= MW || ny >= MH || flow[j] > -1 || !walk(nx, ny) || ch(nx, ny) === 'C') return; flow[j] = d; q.push(j); }); } }   /* columns are walkable but never on the path: a zombie routed through the pillar core just stood there grinding against it */
    ...
    function resetRun(){
      ...
      P = { x:START.x, y:START.y, a:START.a, p:0 }; footY = 0; special = false; fogK = 1; slowT = 0; zombies = []; pickups = []; grenades = []; wave = 0; toSpawn = 0; brutes = 0; spawnT = 1; alive = 0; kills = 0; points = 500; total = 0; hp = 100; maxhp = 100; regenT = 0; doors = {}; power = false; perks = {}; weapons = []; slot = 0; fireT = 0; reloadT = 0; reloading = false; held = {}; mouseDown = false; trigger = false; shake = 0; flash = 0; hitM = 0; hitKill = false; dmgFlash = 0; whiteFlash = 0; banner = ''; bannerT = 0; over = false; puX2 = 0; puInsta = 0; gren = 4; grenT = 0; meleeT = 0; melee = 0; msg = ''; msgT = 0; boxRoll = 0; boxName = ''; boxLast = ''; boxPick = null; swapT = 0; prompt = null; t = 0; bob = 0; stepPh = 0; between = 0; recoil = 0; dragX = null; dragY = null; flowKey = ''; overMsg = '';
    ...
    function newTrain(){ train = { grp:R.train.grp, nose:R.train.nose, tail:R.train.tail, x:-300, v:0, state:'away', next:22, t:0, braking:false, doors:0, released:false, menu:false }; train.grp.visible = false; return train; }
    /* ---------- the line. One chunk is on the scene at a time: the station you stand in, or a stretch of tunnel; the renderer keeps the ones you
       have seen warm off-scene (part 06). What the run remembers about a place (the gates you paid for) is kept out here, so walking back finds it
       as you left it. A tunnel is walked stretch by stretch with no banner between them, and the Conductor crosses the stretches with you. ---------- */
    ...
    function loadChunk(c, place, ride, quiet){
      var prev = chunk, seam = c.t === 'tunnel' && prev && prev.t === 'tunnel';   /* stretch to stretch: the same dark, no banner, the hunt goes on */
      if(prev && prev.t === 'station'){ var was = STATIONS[prev.i].id, keep = {}; Object.keys(doors).forEach(function(k){ keep[k] = 1; }); opened[was] = keep; }
      var carry = seam && conductor && !conductor.dead ? { hp:conductor.hp } : null;   /* the Conductor follows you down the tunnel, wounds and all; he never climbs onto a platform */
      chunk = c; var def = chunkDef(c); if(c.t === 'station'){ stationIdx = c.i; visited[def.id] = 1; }
      genMap(def, c.seg || 0); R.rebuild({ boxHere:c.t === 'station' && boxAt === c.i });   /* the renderer takes the old chunk's bodies off the scene first: it needs the lists to find them */
      toSpawn += alive - (carry ? 1 : 0); alive = 0; zombies = []; pickups = []; grenades = []; conductor = null; transit = 0; doors = {}; prompt = null; boxRoll = 0; bearT = 0; jumpZ = 0; vz = 0;
      newTrain();
      ...
      if(power) R.powerOn();
      if(c.t !== 'tunnel'){ condT = 0; passT = 0; } else if(!seam){ condT = 4; passT = 16 + Math.random()*12; }
      ...
      if(carry) spawnConductor(carry);
      flowKey = ''; spawnT = Math.max(spawnT, .8); regenT = Math.max(regenT, .5);
      if(!quiet && !seam){ banner = (c.t === 'station' ? def.name : 'THE TUNNEL').toUpperCase(); bannerT = 2.6; say(c.t === 'station' ? def.name+'  \u00b7  '+def.sub : 'The tunnel. The train does not stop for you.', 3.2); }
      status();
    }
    ...
    function spawnTransit(x, kind){ var hp0 = zombieHp()*1.7, m = makeZombie(0, false, kind), ty = PIT.z1 + 2.3; alive++; transit++;
      var z = { x:x, y:TRACK_Z + 1.2, tx:x, ty:ty, fx:x, fy:TRACK_Z + 1.2, hp:hp0, maxhp:hp0, speed:kind === 'nypd' ? Math.min(3.2, 2.3 + wave*.04) : Math.min(2.2, 1.4 + wave*.05), dmg:kind === 'nypd' ? 30 : 38, brute:false, wraith:false, runner:kind === 'nypd', transit:true, kind:kind, v:0, anim:Math.random()*6, rise:1.3, rise0:1.3, stairs:false, atk:0, cool:0, dead:false, deadT:0, deadT0:2.2, flash:0, m:m, r:m.r, h:m.h, headY:m.headY, fy2:TRAIN_FLOOR, face:Math.PI/2, fallDir:1 };
    ...
    /* the Conductor: he works the tunnels with a lantern, he is faster than you and he does not lose the scent. Put him down and he leaves ammunition.
       The train does not touch him. With carry (from loadChunk) he arrives in the next stretch with the hp he left the last one on, quietly. */
    function spawnConductor(carry){ if(conductor) return; var m = makeZombie(7, false, 'conductor'), hp0 = carry ? carry.hp : 2200 + wave*500, x = clamp(P.x + (P.x < MW/2 ? 1 : -1)*rnd(16, 24), 2.5, MW - 2.5);
      var z = { x:x, y:TRACK_Z + .5, tx:x, ty:TRACK_Z + .5, fx:x, fy:TRACK_Z + .5, hp:hp0, maxhp:carry ? 2200 + wave*500 : hp0, speed:3.35, dmg:40, brute:false, wraith:false, runner:false, boss:true, kind:'conductor', v:0, anim:0, rise:.8, rise0:.8, stairs:false, atk:0, cool:0, dead:false, deadT:0, deadT0:2.6, flash:0, m:m, r:m.r, h:m.h, headY:m.headY, fy2:PIT_Y, face:0, fallDir:1 };
      m.grp.position.set(z.x, z.fy2, z.y); R.scene.add(m.grp); zombies.push(z); conductor = z; alive++; if(carry) return; snd.wraith(); say('A lantern is coming up the tunnel', 3.4); banner = 'THE CONDUCTOR'; bannerT = 2.6; }
    ...
    function killZombie(z, src){ z.dead = true; z.deadT = z.deadT0; z.fallDir = Math.random() < .8 ? 1 : -1; alive--; kills++; hitKill = true;
      ...
      if(z.boss){ conductor = null; condT = 40; addPoints(500); pickups.push({ x:z.x, y:z.y, kind:'ammo', t:40 }); say('The Conductor is down', 3); banner = 'CONDUCTOR DOWN'; bannerT = 2.4; } R.decal(z.x, z.fy2, z.y, .9 + Math.random()*.6); R.blood(z, 18); if(src !== 'train') snd.kill();
    ...
    function trainCrush(tr){ if(tr.v <= 1.5) return; var x0 = tr.x + tr.tail - .5, x1 = tr.x + tr.nose + 1.2;
      zombies.forEach(function(z){ if(!z.dead && !z.boss && Math.abs(z.y - TRACK_Z) < 2.1 && z.x > x0 && z.x < x1 && ch(z.x|0, z.y|0) !== 'W'){ addPoints(50); killZombie(z, 'train'); z.fallDir = 1; } });
    ...
    g.update = function(dt){
      if(mode === 'menu'){ t += dt; ambientTick(dt); updateTrain(dt); updateAttract(dt); prewarmT -= dt; if(prewarmT <= 0 && (t > 7 || MODEL_KEYS.every(function(k){ return MODELS[k] || MODELS.failed[k]; }))){ prewarmT = .7; if(!R.prewarm() && !actorsWarm){ actorsWarm = true; R.warmActors(); } } return; }   /* the menu is when the other chunks get built and their shaders compiled: once the models are in (or have failed), or seven seconds in regardless */
      t += dt; fade += (fadeTo - fade)*Math.min(1, dt*2.6);
      if(!riding && fade < .1 && t > 1.5){ prewarmT -= dt; if(prewarmT <= 0){ prewarmT = .7; R.prewarm(); } }   /* a run started before the menu finished warming the line picks it up here, one chunk every .7 s, so no station is ever built and uploaded on the frame you arrive in it */
      if(dying > 0){ dying -= dt; shake = Math.max(shake, dying*.25); if(dying <= 0 && !ended) showEnd(); return; }
      if(ended){ endT -= dt; if(endT <= 0) backToMenu(); return; } var edgeK = clamp(1 - Math.min(P.x, MW - P.x)/9, 0, 1), fogTo = special ? .45 : chunk && chunk.t === 'tunnel' ? .4 - edgeK*.24 : P.y >= PIT.z0 && P.y < PIT.z1 + 1 && !riding ? 1 - edgeK*.72 : 1; fogK += (fogTo - fogK) * Math.min(1, dt*(fogTo > fogK ? .7 : 1.5)); slowT = Math.max(0, slowT - dt);   /* tunnels: fog closes to near-black at either end so the seam swap is invisible, and it lifts slowly on the far side */ fireT -= dt; grenT -= dt; meleeT -= dt; melee = Math.max(0, melee - dt*3); flash = Math.max(0, flash - dt); hitM = Math.max(0, hitM - dt); dmgFlash = Math.max(0, dmgFlash - dt); whiteFlash = Math.max(0, whiteFlash - dt); shake = Math.max(0, shake - dt*2.5); recoil = Math.max(0, recoil - dt*6); bannerT -= dt; msgT -= dt; puX2 = Math.max(0, puX2 - dt); puInsta = Math.max(0, puInsta - dt);
      ...
      var fw = (held.W ? 1 : 0) - (held.S ? 1 : 0), sf = (held.D ? 1 : 0) - (held.A ? 1 : 0), sp = 3.1 * (held.sprint && fw > 0 ? (perks.fleetfoot ? 1.75 : 1.45) : 1) * (slowT > 0 ? .5 : 1);
      ...
      zombies.forEach(function(z){
        ...
        if(z.rise > 0){ z.rise -= dt; var k = clamp(1 - z.rise/z.rise0, 0, 1); z.x = lerp(z.fx, z.tx, k); z.y = lerp(z.fy, z.ty, k); z.climb = z.stairs ? (1 - k)*(1 - k)*1.0 + Math.sin(k*Math.PI)*.12 : 0; z.anim += dt*2; z.face = Math.atan2(z.tx - z.fx, z.ty - z.fy); if(Math.random() < dt*.4) snd.growl(Math.hypot(P.x - z.x, P.y - z.y) < 7); return; }
        ...
        if(z.atk > 0){ z.atk -= dt; z.face = Math.atan2(rx, ry); if(z.atk <= 0){ if(d < (z.boss ? 1.6 : 1.25) && Math.abs(footY - z.fy2) < 1.2){ hurt(z.dmg); if(z.boss){ slowT = .9; shake = Math.max(shake, 1.4); } } z.cool = z.boss ? 2.2 : .9; } return; }   /* the Conductor's lantern swing: it lands hard and slows you, then he needs a moment to recover, so a full sprint can still just pull away */
        if(z.cool > 0) z.cool -= dt; else if(d < (z.boss ? 1.3 : .9) && Math.abs(footY - z.fy2) < 1.2){ z.atk = z.boss ? .28 : .45; return; }   /* the Conductor reaches further and swings faster: a sprinter cannot slip through his wind-up */
        ...
        var kx = z.x|0, ky = z.y|0; for(var ox = -1; ox <= 1; ox++) for(var oy = -1; oy <= 1; oy++){ if(ch(kx + ox, ky + oy) !== 'C') continue; var qx = z.x - (kx + ox + .5), qy = z.y - (ky + oy + .5), qd = Math.hypot(qx, qy); if(qd < .95 && qd > 0){ var w = (1 - qd/.95)*1.5, sgn = (-qy*vx + qx*vy) >= 0 ? 1 : -1; vx += (-qy*sgn*w + qx*w*.4)/qd; vy += (qx*sgn*w + qy*w*.4)/qd; } }   /* pillars: slide around the near side instead of grinding on the core */
        var l2 = Math.hypot(vx, vy) || 1, spd = z.speed * (z.boss && z.cool <= 0 && d < 3.5 && d > 1.2 ? 1.25 : 1); moveCircle(z, z.x + vx/l2*spd*dt, z.y + vy/l2*spd*dt, .28); z.anim += dt * z.speed * 1.4; z.face = Math.atan2(vx, vy);
        if(Math.random() < dt*.05) snd.growl(d < 7);
      });
      zombies.forEach(function(z){ var fl2 = inCar(z.x, z.y) ? TRAIN_FLOOR : floorAt(z.x|0, z.y|0); z.fy2 += (fl2 - z.fy2) * Math.min(1, dt*9); });   /* inside a standing car they stand on its floor, not the track bed under it */
...
/* tigOS arcade, Nightshift part 06: the picture. Renderer, camera and post (bloom), the per-frame animation of everything part 05 decided,
   the DOM HUD over the canvas, the title card with its settings, input, and the peek/dbg hooks the test suite drives. Closes nightshift(). */
    ...
    (function build(){
      ...
      /* the world cache: every chunk built once and kept off-scene between visits (see part 02). The train and the conductor's lantern light are
         scene-level, and a pool of dark pad lights keeps the visible point-light count fixed per graphics tier, so no chunk swap, train arrival or
         boss spawn ever changes the light count and makes Three.js recompile the shaders: that recompile was the lag. */
      var TR = buildTrain(scene), cache = {}, pads = [], lanternL = new THREE.PointLight(0xffb050, 0, 9, 1); scene.add(lanternL);
      for(var pi = 0; pi < 28; pi++){ var pd = new THREE.PointLight(0xffffff, 0, .01, 1); pd.visible = false; scene.add(pd); pads.push(pd); }
      var chunkKey = function(c){ return c.t === 'station' ? 'station:'+c.i : 'tunnel:'+(c.seg || 0); };
      var getWorld = function(key, opts){ var w = cache[key]; if(!w){ var t0 = performance.now(); w = buildWorld(scene, opts); w.key = key; w.buildMs = performance.now() - t0; cache[key] = w; } w.attach(MACHINES, opts); return w; };
      var world = getWorld('station:0', { boxHere:true }); chunk = { t:'station', i:0 };
      var composer = new THREE.EffectComposer(renderer); composer.addPass(new THREE.RenderPass(scene, camera)); var bloom = new THREE.UnrealBloomPass(new THREE.Vector2(320, 180), .22, .3, 1.05);   /* only what is brighter than white glows: tubes, lamps, eyes, panels */ composer.addPass(bloom); var outPass = new THREE.OutputPass(); composer.addPass(outPass);
      /* the menu's CCTV look, done inside the output pass: the CSS filter chain it replaces re-filtered the whole composited canvas at device pixels every frame (six filters plus a blended, animated scanline layer), the single
         most expensive thing on the menu on a weak GPU. Here it is a few lines of the shader that already runs, at render resolution. The CSS stays as the fallback when the composer is off (low tier). */
      var gradeGL = false; try { var om = outPass.material, fs0 = om.fragmentShader; if(fs0.indexOf('#ifdef SRGB_TRANSFER') >= 0){ om.uniforms.cctv = { value:0 }; om.uniforms.cctvT = { value:0 }; om.uniforms.cctvS = { value:1 };
        om.fragmentShader = fs0.replace('uniform sampler2D tDiffuse;', 'uniform sampler2D tDiffuse; uniform float cctv; uniform float cctvT; uniform float cctvS;').replace('#ifdef SRGB_TRANSFER', 'if(cctv > 0.0){ vec3 c = gl_FragColor.rgb; float l = dot(c, vec3(.2126, .7152, .0722)); c = mix(c, vec3(l), .7); c = mix(c, l*vec3(.72, 1.0, .78), .55); c = (c - .5)*1.2 + .5; c *= .86; float scan = 1.0 - .16*step(mod(gl_FragCoord.y/cctvS + cctvT, 3.0), 1.0); gl_FragColor.rgb = mix(gl_FragColor.rgb, max(c, vec3(0.0))*scan, cctv); }\n#ifdef SRGB_TRANSFER');
        om.needsUpdate = true; gradeGL = true; } } catch(e){ gradeGL = false; }
      ...
      /* the tier sets the picture; the governor below may take it down a notch or three when the frames say the machine cannot keep up (auto.level 1..3: .8x, .62x, .5x pixels, bloom off at 3) */
      /* the CCTV grade: in the output pass when the composer runs, the CSS filter chain otherwise (low tier, or a bundle without the hook) */
      function grade(on){ var gl = gradeGL && bloom.enabled; api.canvas.classList.toggle('cctv', on); api.canvas.classList.toggle('css-grade', on && !gl); if(gradeGL) outPass.material.uniforms.cctv.value = on && gl ? 1 : 0; if(menuEl) menuEl.classList.toggle('gl-grade', gl); }
      var auto = { level:0, samples:[], last:0, p50:0, windows:0 }, AUTO_K = [1, .8, .62, .5];
      function applyGfx(){ var tier = soft ? 'low' : SET.gfx, dpr = window.devicePixelRatio || 1, pr = (soft ? .3 : { low:.55, medium:.8, high:Math.min(1.5, dpr), ultra:Math.min(2, dpr) }[tier])*AUTO_K[auto.level]; renderer.setPixelRatio(pr); composer.setPixelRatio(pr); if(gradeGL) outPass.material.uniforms.cctvS.value = pr;
        bloom.enabled = (tier !== 'low' && auto.level < 3) || /[?&]gl=1/.test(location.search); bloom.strength = tier === 'ultra' ? .28 : .22; applyLights(); if(vw > 2) R.resize(vw, vh); if(api.canvas.classList.contains('cctv')) grade(true); }
      /* the tier's light budget, exactly: the chunk's brightest lights up to the budget, dark pads for the rest (a tunnel has nine lights, a station near thirty) */
      function applyLights(){ var tier = soft ? 'low' : SET.gfx, nl = { low:8, medium:12, high:17, ultra:28 }[tier], n = 0; world.lights.forEach(function(o, i){ o.l.visible = i < nl; if(o.l.visible) n++; }); pads.forEach(function(pd, i){ pd.visible = i < nl - n; }); }
      R = { scene:scene, camera:camera, renderer:renderer, world:world, train:TR, fresh:true, soft:function(){ return soft; }, bloom:bloom, cache:cache, upload:function(root){ uploadScene(root); }, auto:function(){ return { level:auto.level, p50:+auto.p50.toFixed(1), pr:renderer.getPixelRatio() }; }, gradeGL:function(){ return gradeGL; },
        ...
        /* down the line: this chunk steps off the scene and the next steps on, built now if it is the first visit and otherwise straight from the cache.
           The viewmodel lives on the camera and the particle pools, the train and the box's display guns are scene-wide, so nothing else moves. */
        rebuild:function(opts){ clearActors(); decals.clear(); Object.keys(boxGuns).forEach(function(k){ boxGuns[k].grp.visible = false; });
          world.detach(); world = getWorld(chunkKey(chunk), opts || {}); R.world = world; applyLights(); paintWorld(); forgeT = 0; vendT = {}; powerSweep = power ? 99 : 0; return world; },
        /* while the menu is up: build the chunks not yet seen, one a call, and have their shaders compiled in the background so the first walk down the line
           costs nothing. The map globals are the current chunk's again before this returns. */
        prewarm:function(){ var keys = ['station:1', 'station:2', 'station:3', 'tunnel:0', 'tunnel:1'].filter(function(k){ return !cache[k]; }); if(!keys.length) return null;
          var k = keys[0], parts = k.split(':'), cur = CUR, curSeg = CUR.seg || 0, here = !!(BOXM && BOXM.here);
          if(parts[0] === 'station') genMap(STATIONS[+parts[1]]); else genMap(TUNNEL, +parts[1]);
          var t0 = performance.now(), w = buildWorld(scene, { boxHere:false }); w.key = k; w.buildMs = performance.now() - t0; cache[k] = w;
          world.detach(); var keep = world; world = w; applyLights(); compileScene();   /* the light count is the tier's, so the programs it makes are the ones the visit will use */
          uploadScene(w.root);   /* compile makes the programs but not the vertex buffers: a first draw of a station uploaded ~600 geometries in one 50-90 ms frame, the hitch you felt as the train pulled in. One render into an offscreen target, culling off and every hidden part shown, puts them on the GPU now */
          w.detach(); world = keep; genMap(cur, curSeg); world.attach(MACHINES, { boxHere:here }); applyLights(); w.textures.forEach(function(tx){ try { renderer.initTexture(tx); } catch(e){} }); return k; },   /* and its canvases go up to the GPU now, not on the first frame there */
        /* a model landed after worlds were built: those worlds hold stand-ins, so they go, and the one on the scene is rebuilt in place with the run's gates and power */
        invalidate:function(){ Object.keys(cache).forEach(function(k){ if(cache[k] !== world){ cache[k].destroy(); delete cache[k]; } }); var old = world; delete cache[old.key]; old.destroy();
          world = getWorld(chunkKey(chunk), { boxHere:!!(BOXM && BOXM.here) }); R.world = world; applyLights(); paintWorld(); Object.keys(doors || {}).forEach(function(k){ R.openDoor(k, true); }); if(power) R.powerOn(); return Object.keys(cache); },
        /* one body of every kind of the dead, a pickup and its label, put on the scene for a moment so their programs compile and their skins upload before the
           first one ever spawns. Runs again when a model lands, since the Blender bodies carry their own materials. */
        warm:null,
        warmActors:function(){ var tmp = new THREE.Group(), bodies = []; ['walker', 'runner', 'brute', 'mta', 'nypd', 'conductor', 'wraith'].forEach(function(k){ try { var m = makeZombie(0, false, k); m.grp.traverse(function(o){ o.visible = true; }); bodies.push(m); tmp.add(m.grp); } catch(e){} });   /* every optional part shown: compile only visits visible meshes, and a torn sleeve or ribs that happened to be hidden here would otherwise compile on its first spawn, possibly mid-seam */
          var pm = new THREE.Mesh(puGeo, new THREE.MeshStandardMaterial({ color:0xffffff, emissive:0xffffff, emissiveIntensity:1 })), sp = new THREE.Sprite(new THREE.SpriteMaterial({ map:puTex.ammo, transparent:true, depthWrite:false })); tmp.add(pm); tmp.add(sp);
          if(R.warm){ scene.remove(R.warm); R.warm.traverse(function(o){ if(o.isMesh || o.isSprite){ [].concat(o.material).forEach(function(mt){ mt.dispose(); }); } }); } tmp.position.set(MW/2, -50, MH/2); scene.add(tmp); R.warm = tmp; compileScene(); uploadScene(tmp); uploadScene(TR.grp); uploadScene(world.root);   /* and their vertex buffers, the train's and the first station's: parts culled or hidden until now (the far cab's glass, a hard hat) otherwise upload on the frame they first show */
          ztex().forEach(function(tx){ ['skin', 'cloth', 'pants'].forEach(function(n){ try { renderer.initTexture(tx[n]); } catch(e){} }); }); Object.keys(puTex).forEach(function(k){ try { renderer.initTexture(puTex[k]); } catch(e){} });
          return bodies.length; },   /* the bodies stay in the scene, 50 m under the map: three.js releases a program the moment its last material is disposed, so warming and then disposing pinned nothing, and the actor shaders came back to compile on the first spawn after a quiet moment (or at a seam). Kept alive, every compileScene at a chunk load recompiles them for that chunk's light count */
        ...
        flicker:flickStep, frame:function(dt){
          ...
          if(mode === 'menu'){   /* the security camera: high in the west corner of the platform, a slow pan across the platform and the tracks, a little roll */
            camera.position.set(STATION.x0 + .9, CEIL_HI - .25, STATION.z1 - .8); tmpV.set(30 + Math.sin(t*.13)*7, .4, 7.2); camera.lookAt(tmpV); camera.rotation.z += .035; gunPark(); }
            ...
            camera.position.set(P.x, footY + EYE - ez*(EYE - .26), P.y); camera.rotation.set(-.2 - ez*.55, Math.atan2(-Math.cos(P.a), -Math.sin(P.a)) + ez*.3, ez*1.2); gunPark(); }
          else { var rs = riding ? 1 : 0; camera.position.set(P.x + rnd(-1, 1)*sk + rs*Math.sin(t*5.7)*.012, eye + rnd(-1, 1)*sk + rs*Math.sin(t*8.3)*.014, P.y + rnd(-1, 1)*sk + rs*Math.sin(t*4.1)*.01); camera.rotation.set(P.p + rnd(-1, 1)*sk*.5, Math.atan2(-Math.cos(P.a), -Math.sin(P.a)), Math.sin(bob)*.006 + rnd(-1, 1)*sk*.3 + rs*Math.sin(t*3.3)*.01); if(forgeT > 0) gunPark(); }
          /* the gun: bob, recoil, reload dip, swap slide, melee shove */
          if(gun && mode !== 'menu' && !(dying > 0 || ended || forgeT > 0)){ var rp = reloading ? Math.sin((1 - reloadT/stat(gunW).reload) * Math.PI) : 0, sw = swapT > 0 ? Math.sin(Math.PI*swapT/.34) : 0;
          ...
          var cz = conductor && !conductor.dead ? conductor : null; if(cz && cz.m.lantern){ cz.m.lantern.getWorldPosition(tmpV); lanternL.position.copy(tmpV); lanternL.intensity = 1.5 + Math.sin(t*9)*.5; } else lanternL.intensity = Math.max(0, lanternL.intensity - dt*3);   /* the one lantern light in the scene rides with the Conductor */
          ...
          var wt = TR; if(wt){ wt.lights.position.x = train ? train.x : -200; var dk2 = train ? train.doors : 0; wt.doors.forEach(function(dr){ dr.position.x = dr.userData.x0 + dr.userData.side*dk2*.72; }); var cab = riding ? 1 : (train && train.doors > .05 ? train.doors : 0); wt.cabin.intensity = cab*.55; wt.carLights.forEach(function(cl){ cl.intensity = cab*1.5; }); if(train) wt.cabin.position.x = clamp(P.x - train.x, 0, wt.nose); }   /* the car light follows you down the consist */
          var now = performance.now(); if(auto.last && !document.hidden && !soft){ var gap = now - auto.last; if(gap < 200){ auto.samples.push(gap); if(auto.samples.length >= 120){ auto.samples.sort(function(a, b){ return a - b; }); auto.p50 = auto.samples[60]; auto.samples.length = 0; auto.windows++;
            if(auto.p50 > 24 && auto.level < 3 && auto.windows > 2){ auto.level++; applyGfx(); try { console.info('Nightshift: frames at '+auto.p50.toFixed(0)+' ms, picture stepped down to level '+auto.level); } catch(e){} } } } } auto.last = now;   /* two seconds of frames under 42 fps (after the first four seconds, when shaders are still compiling), measured on the rAF gap the browser actually gave us, steps the picture down; it never steps back up in a session, so the picture does not pump */
          ...
          TR.hlight.intensity = train && train.grp.visible ? 9 : 0;
          /* fog thickens and cools when the wraiths come */
          var fd = mode === 'menu' ? .021 : .042/fogK; scene.fog.density += (fd - scene.fog.density)*Math.min(1, dt*2); scene.fog.color.setHex(0x04050a).lerp(new THREE.Color(chunk && chunk.t === 'tunnel' && !special ? 0x07080c : 0x1a2238), 1 - fogK); scene.background.copy(scene.fog.color);
          blood.update(dt, floorAt2); sparks.update(dt, floorAt2); tracers.update(dt); beams.update(dt);
          camera.updateMatrixWorld();
          if(gradeGL && mode === 'menu') outPass.material.uniforms.cctvT.value = (t*9)%3;
        ...
        menu:function(on){ grade(on); if(hudEl) hudEl.hidden = true; if(menuEl){ menuEl.hidden = !on; if(on) showPanel('main'); } },
        dispose:function(){ if(gun){ camera.remove(gun.grp); gun.dispose(); } Object.keys(boxGuns).forEach(function(k){ boxGuns[k].dispose(); }); blood.dispose(); sparks.dispose(); tracers.dispose(); beams.dispose(); decals.dispose(); grenGeo.dispose(); puGeo.dispose(); Object.keys(cache).forEach(function(k){ cache[k].destroy(); }); TR.dispose(); scene.traverse(function(o){ if(o.geometry && o.geometry.dispose) o.geometry.dispose(); }); if(warmRT) warmRT.dispose(); composer.dispose(); renderer.dispose(); try { renderer.forceContextLoss(); } catch(e){} } };
      function floorAt2(x, z){ return floorAt(x|0, z|0); }
      /* shaders are keyed by the render target they draw into: the game draws through the bloom composer's buffer, so compile with that bound or the
         programs made here are the wrong variant and the first real frame compiles them all again */
      var warmRT = null;
      function uploadScene(root){ try { if(!warmRT) warmRT = new THREE.WebGLRenderTarget(4, 4); var culled = [], hidden = []; root.traverse(function(o){ if(o.isMesh && o.frustumCulled){ o.frustumCulled = false; culled.push(o); } if(o.visible === false && !o.isLight){ var hasL = false; o.traverse(function(q){ if(q.isLight) hasL = true; }); if(!hasL){ o.visible = true; hidden.push(o); } } });   /* never a hidden light: the light count is the tier's budget, and one more would compile a whole second set of programs */ renderer.setRenderTarget(bloom.enabled ? composer.readBuffer : warmRT); renderer.render(scene, camera); culled.forEach(function(o){ o.frustumCulled = true; }); hidden.forEach(function(o){ o.visible = false; }); } catch(e){} finally { try { renderer.setRenderTarget(null); } catch(e2){} } }
      function compileScene(){ try { renderer.setRenderTarget(bloom.enabled ? composer.readBuffer : null); if(renderer.compileAsync) renderer.compileAsync(scene, camera).catch(function(){}); else renderer.compile(scene, camera); } catch(e){} finally { try { renderer.setRenderTarget(null); } catch(e2){} } }
      function gunPark(){ if(gun){ gun.grp.position.set(0, -4, 2); gun.flash.forEach(function(f){ f.visible = false; }); gun.light.intensity = 0; } }   /* out of frame behind the camera, never invisible: its two lights stay in the count */
    ...
    /* every HUD write goes through a change guard: setting hidden, className or a style to the value it already has still costs a mutation and a style pass per frame, sixty times a second */
    function setS(k, prop, v){ var kk = 's_'+k+prop; if(last[kk] !== v){ last[kk] = v; hud[k].style[prop] = v; } }
    function setC(k, cls){ var kk = 'c_'+k; if(last[kk] !== cls){ last[kk] = cls; hud[k].className = cls; } }
    function setH(k, hid){ var kk = 'h_'+k; if(last[kk] !== hid){ last[kk] = hid; hud[k].hidden = hid; } }
    function drawHud(){
      ...
      var wpn = cur(), stt = stat(wpn); setT('wname', stt.name.toUpperCase() + (wpn.up ? '  \u2726' : '')); setS('wname', 'color', wpn.up ? '#ff8a00' : ''); setT('ammo', reloading ? 'RELOADING' : wpn.mag + ' | ' + wpn.res); setS('ammo', 'color', wpn.mag === 0 && !reloading ? '#ff5f57' : '');
      setT('gren', '\u2B22 '+gren); setS('gren', 'opacity', gren ? 1 : .35); setT('other', weapons.length > 1 ? 'Q  '+stat(weapons[1 - slot]).name : '');
      var ba = bannerT > 0 ? Math.min(1, bannerT) : 0; setT('banner', ba ? banner : ''); setS('banner', 'opacity', ba); setC('banner', 'ns-banner'+(banner.indexOf('WAVE') === 0 ? ' red' : ''));
      ...
      var ps = prompt ? '['+(touch ? 'USE' : 'F')+']  '+prompt.txt+(prompt.cost ? '  \u00b7  '+prompt.cost : '') : ''; setT('prompt', ps); setH('prompt', !ps); setC('prompt', 'ns-prompt'+(prompt && (prompt.deny || points < prompt.cost) ? ' no' : ''));
      setT('msg', msgT > 0 ? msg : ''); setS('msg', 'opacity', msgT > 0 ? Math.min(1, msgT*2) : 0);
      setT('box', boxRoll > 0 ? boxName.toUpperCase() : ''); setH('box', boxRoll <= 0);
      setH('hit', hitM <= 0); setC('hit', 'ns-hit'+(hitKill ? ' kill' : '')); var gap = (6 + Math.round(recoil*70)/10)+'px'; if(last.gap !== gap){ last.gap = gap; hud.cross.style.setProperty('--gap', gap); }
    ...
    /* the tunnel bulbs: each on its own bad circuit, mostly dark with a lamp coming and going; a stutter now and then while it is on. Only intensity moves, so the light count the renderer budgets against never changes */
    function flickStep(dt){ if(!(chunk && chunk.t === 'tunnel')) return 0; var n = 0; R.world.flick.forEach(function(f){ f.t -= dt; if(f.t <= 0){ f.on = !f.on; f.t = f.on ? rnd(.5, 3.2) : rnd(.05, 1.1); n++; } var stut = f.on && Math.random() < .08 ? .35 : 1; f.l.intensity = f.on ? f.base*stut : 0; f.m.emissiveIntensity = f.on ? 2.2*stut : .05; }); return n; }
    ...
    var onModel = function(k, sc){ if(!R) return; if(k === 'train'){ R.train.setModel(sc); R.upload(R.train.grp); } if(k === 'zombie' || k === 'wraith') R.warmActors(); if(k === 'props') R.invalidate(); }; MODELS.listeners.push(onModel);
    ...
    g.peek = function(){ return { station:chunk && chunk.t === 'station' ? STATIONS[chunk.i].id : null, stationName:CUR ? CUR.name : null, chunk:chunk ? (chunk.t === 'station' ? 'station:'+chunk.i : 'tunnel:'+chunk.from+':'+chunk.seg) : null, tunnel:!!(CUR && CUR.tunnel), visited:Object.keys(visited).length,
      ...
      special:special, fogK:+fogK.toFixed(2), wave:wave, kills:kills, points:points, total:total, hp:hp, alive:alive, toSpawn:toSpawn, zombies:zombies.length, weapon:stat(cur()).name, mag:cur().mag, res:cur().res, doors:Object.keys(doors).length, power:power, perks:Object.keys(perks), x:P.x, y:P.y, a:P.a, pitch:P.p, footY:+footY.toFixed(2), over:over, overMsg:overMsg, between:+between.toFixed(2), reloading:reloading, swap:+swapT.toFixed(3), boxRoll:+boxRoll.toFixed(2), boxName:boxName, prompt:prompt ? prompt.txt : null, promptCost:prompt ? prompt.cost : null, dmg:stat(cur()).dmg, beams:R ? R.beamsLive() : 0, gun:gunScreen(), gren:gren, puX2:+puX2.toFixed(1), puInsta:+puInsta.toFixed(1), soft:R ? R.soft() : null, bloom:R ? R.bloom.enabled : null, auto:R ? R.auto() : null, gradeGL:R ? R.gradeGL() : null, lights:R ? R.world.lights.filter(function(o){ return o.l.visible; }).length : 0, gfx:SET.gfx, fov:SET.fov, sens:SET.sens, train:train ? { state:train.state, x:+train.x.toFixed(1), v:+train.v.toFixed(2), doors:+train.doors.toFixed(2), nose:+(train.x + train.nose).toFixed(1), visible:train.grp.visible, next:+train.next.toFixed(1) } : null, banner:bannerT > 0 ? banner : '', gates:R ? Object.keys(R.world.doors).filter(function(k){ return R.world.doors[k].grp.visible; }).length : 0, hud:hudEl ? !hudEl.hidden : false, models:{ wraith:!!MODELS.wraith, train:!!MODELS.train, zombie:!!MODELS.zombie, guns:!!MODELS.guns, props:!!MODELS.props, trainLive:!!(R && R.train.model), gunGlb:!!(R && R.gunGlb()), failed:MODELS.failed }, mode:mode, menu:menuEl ? { open:!menuEl.hidden, panel:menuPanel, sel:menuSel, item:menuItems()[menuSel] ? menuItems()[menuSel].getAttribute('data-act') : null, clock:menuEl.querySelector('[data-clock]').textContent, attract:attract && attract.z ? { x:+attract.z.x.toFixed(2), y:+attract.z.y.toFixed(2), phase:attract.z.phase } : null, cctv:api.canvas.classList.contains('cctv') } : null, jump:+jumpZ.toFixed(2), vz:+vz.toFixed(2), slot:slot, weapons:weapons.map(function(w){ return stat(w).name; }), photo:A.photo, inv:!!SET.inv,
      list:zombies.map(function(z){ return { x:z.x, y:z.y, hp:z.hp, dead:z.dead, rise:z.rise, climb:+(z.climb || 0).toFixed(2), st:!!z.stairs, brute:z.brute, wraith:z.wraith, runner:z.runner, atk:z.atk, fy:+z.fy2.toFixed(2), glb:!!z.m.glb, kind:z.kind || null, human:!!z.m.human }; }) }; };
    g.dbg = { locate:function(){ return LOCATE; }, trainInfo:function(){ var n = 0; R.train.lights.traverse(function(o){ if(o.isPointLight) n++; }); return { stand:R.train.stand.visible, inner:R.train.inner.visible, carLights:R.train.carLights.length, lightsInGroup:n, model:R.train.model, parent:R.train.grp.parent === R.scene }; },
      renderer:function(){ return R.renderer; }, world:function(){ return R.world; }, sceneLights:function(){ var n = 0; R.scene.traverseVisible(function(o){ if(o.isLight) n++; }); return n; }, cached:function(){ return Object.keys(R.cache).sort(); }, cacheWorld:function(k){ return R.cache[k] || null; }, uploaded:function(k){ var w = R.cache[k], n = 0, up = 0; if(!w) return null; w.root.traverse(function(o){ if(o.geometry){ n++; var l = o.geometry._listeners; if(l && l.dispose && l.dispose.length) up++; } }); return { meshes:n, uploaded:up }; },   /* the renderer hangs a dispose listener on every geometry it has put on the GPU, so this is the prewarm's upload coverage */ prewarm:function(){ return R.prewarm(); }, invalidate:function(){ return R.invalidate(); }, warmActors:function(){ return R.warmActors(); }, warmBodies:function(){ return R.warm && R.warm.parent === R.scene ? R.warm.children.filter(function(o){ return o.isGroup; }).length : 0; }, gpu:function(){ var r = R.renderer; return { programs:r.info.programs.length, textures:r.info.memory.textures, geometries:r.info.memory.geometries }; }, buildTime:function(){ var o = {}; Object.keys(R.cache).forEach(function(k){ o[k] = +R.cache[k].buildMs.toFixed(1); }); return o; }, tick:function(sec){ var n = Math.round((sec || 1)*60); for(var i = 0; i < n; i++) g.update(1/60); return t; }, station:function(i){ loadChunk({ t:'station', i:((i % STATIONS.length) + STATIONS.length) % STATIONS.length }, null, 0, true); return CUR.id; },
      flicker:function(sec){ var n = Math.round((sec || 1)*60), k = 0; for(var i = 0; i < n; i++) k += flickStep(1/60); return { toggles:k, states:R.world.flick.map(function(f){ return { on:f.on, i:+f.l.intensity.toFixed(2), e:+f.m.emissiveIntensity.toFixed(2), vis:f.l.visible }; }) }; }, tunnel:function(k, from){ loadChunk({ t:'tunnel', from:from === undefined ? stationIdx : from, seg:k || 0 }, { x:MW/2, y:TRACK_Z + .5 }, 0, true); return CUR.id; },
      ...
      walkAt:function(x, z){ return { walk:walk(x, z), door:doorAt(x), flow:flow[x + z*MW], ch:ch(x, z), open:trainOpen(), blocks:trainBlocks(x) }; }, teleport:function(x, y, a, p){ P.x = x; P.y = y; if(a !== undefined) P.a = a; if(p !== undefined) P.p = p; footY = floorAt(x|0, y|0); }, killAll:function(){ zombies.forEach(function(z){ if(!z.dead){ z.dead = true; z.deadT = .1; alive--; } }); alive = Math.max(0, alive); }, wave:function(n){ zombies.forEach(function(z){ if(!z.dead){ z.dead = true; z.deadT = .1; } }); alive = 0; between = 0; startWave(n); }, hurt:function(n){ hurt(n); }, power:function(){ power = true; R.powerOn(); }, train:function(){ if(train){ train.next = 0; if(wave < 2) wave = 2; } }, trainAt:function(x, v, state){ train.grp.visible = true; train.x = x; train.v = v; train.state = state || 'coming'; }, gfx:function(t){ SET.gfx = t; R.applySettings(); }, spawnStairs:function(i){ var st = SPAWNS.filter(function(s){ return s.stairs; }); if(!st.length) return null; var keep = SPAWNS; SPAWNS = [st[(i || 0) % st.length]]; toSpawn++; spawn(); SPAWNS = keep; var z = zombies[zombies.length - 1]; return { x:z.x, y:z.y, tx:z.tx, ty:z.ty, fx:z.fx, fy:z.fy, rise:z.rise, stairs:z.stairs }; }, spawnAt:function(x, y, hold){ var s = { x:x, y:y, fx:x, fy:y, stairs:false }; var keep = SPAWNS; SPAWNS = [s]; toSpawn++; spawn(); SPAWNS = keep; var z = zombies[zombies.length - 1]; if(z){ z.rise = 0; z.x = x; z.y = y; z.hold = !!hold; } return zombies.length; } };
    return g;
  }
