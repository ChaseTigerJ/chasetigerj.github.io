  ...
  RENDER.games = function(w){
    ...
    function play(id){
      ...
      stage.addEventListener('pointerup', function(e){ if(!cur) return; var p = pt(e); cur.pointer('up', p.x, p.y, e);
        if(sw && (g.pad === 'dirs' || g.pad === 'stack' || g.pad === 'swipe')){ var dx = e.clientX - sw.x, dy = e.clientY - sw.y, k = null; if(Math.hypot(dx, dy) > 24) k = Math.abs(dx) > Math.abs(dy) ? (dx > 0 ? 'ArrowRight' : 'ArrowLeft') : (dy > 0 ? 'ArrowDown' : 'ArrowUp'); else if(g.pad === 'stack' && e.pointerType === 'touch') k = 'ArrowUp'; if(k){ cur.key(k, true); cur.key(k, false); } } sw = null; });
